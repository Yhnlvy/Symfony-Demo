...
class BlogControllerTest extends WebTestCase
{
    ...
    {
        $client = static::createClient();
        $crawler = $client->request('GET', '/fr/blog/posts/lorem-ipsum-dolor-sit-amet-consectetur-adipiscing-elit');
        
        // Title is displayed in a <h1> html tag
        $postTitle = 'Lorem ipsum dolor sit amet consectetur adipiscing elit';
        $foundTitle = $crawler->filter('h1')->text();
        $this->assertEquals($foundTitle, $postTitle);
        
        // The comments list is present
        $commentsCount = $crawler->filter('h3')->text();
        $commentsCount = explode(" ", $commentsCount);
        $commentsCount = (int) $commentsCount[0];
        $commentsDivCount = $crawler->filter('div[class="row post-comment"]')->count();
        $this->assertEquals($commentsCount, $commentsDivCount);
